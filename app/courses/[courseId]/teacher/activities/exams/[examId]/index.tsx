...
export default function TeacherExamPage() {
  ...
  const handleEditExam = async () => {
    ...
    try {
      if (teacherExam) {
        await updateActivity(courseId, teacherExam.activity, examDetails);
        setTeacherExam({
          ...teacherExam,
          activity: {
  ...
  const handlePublishActivity = async () => {
    ...
    try {
      if (teacherExam) {
        await publishActivity(courseId, teacherExam.activity);
        setTeacherExam({
          ...teacherExam,
          visible: true,
        });
  ...
  const handleDeleteActivity = async () => {
    ...
    try {
      if (teacherExam) {
        await deleteExam(courseId, teacherExam.activity.resourceId);
        setTeacherExam(null);
        router.back();
      }
    } catch (error) {
  ...
  return (
    <>
      <Appbar.Header>
        ...
        />
        <Appbar.Content
          title={isEditing ? "Editado examen" : "Información del examen"}
        />
